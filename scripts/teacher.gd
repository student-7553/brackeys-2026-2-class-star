...
var _target_student: Student
var _waypoints: Array[Vector3] = []
var _wait_time := 0.0


func start_moving(students: Array[Student]) -> void:
	_students = students
	_pick_next_student()
...
func _pick_next_student() -> void:
	_waypoints.clear()
	if _students.is_empty():
		return

	var student: Student = _students.pick_random()
	if _students.size() > 1:
		while student == _target_student:
			student = _students.pick_random()

	if student == null or not is_instance_valid(student):
		return

	_target_student = student
	_waypoints = _aisle_path(_beside_table(student))


func _beside_table(student: Student) -> Vector3:
	var table := student.table
	if table == null or not is_instance_valid(table):
		var fallback := student.global_position
...
func _nearest_aisle(value: float, along_x: bool) -> float:
	...
	for student in _students:
		if student == null or not is_instance_valid(student):
			continue
		for candidate in _aisle_candidates(student, along_x):
...
func _aisle_candidates(student: Student, along_x: bool) -> Array[float]:
	var table := student.table
	if table == null or not is_instance_valid(table):
		var center := student.global_position.x if along_x else student.global_position.z
		var half := (student.WIDTH if along_x else student.LENGTH) * 0.5
		return [center + half + STAND_OFFSET, center - half - STAND_OFFSET]
	...
	if not along_x:
		plus = student.global_position.z + student.LENGTH * 0.5 + STAND_OFFSET
	return [plus, minus]
...
func _arrive_at_table() -> void:
	...
	if is_instance_valid(_target_student):
		var look_pos := _target_student.global_position
		if _target_student.table != null and is_instance_valid(_target_student.table):
			look_pos = _target_student.table.global_position
